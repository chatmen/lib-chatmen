...
import chatmen.core.model.{Favorite, Tweet}
...
case class FavoriteTable[P <: JdbcProfile]()(implicit val driver: P)
    extends Table[Favorite, P] with SlickColumnTypes[P] {
  import api._
  ...
  //--[ テーブル定義 ] --------------------------------------------------------
  class Table(tag: Tag) extends BasicTable(tag, "chatmen_favorite") {

    //Columns
    /* @1 */ def id            = column[Favorite.Id]    ("id",            O.UInt64, O.PrimaryKey, O.AutoInc)
    ...
    type TableElementTuple = (
      Option[Favorite.Id], Tweet.Id, User.Id,
    ...
    def * = (id.?, tid, uid, updatedAt, createdAt) <> (
      ...
      (t: TableElementTuple) => {
        Favorite(t._1, t._2, t._3, t._4,t._5)
      },
      /** The bidirectional mappings : Model => Tuple(table) */
      (v: TableElementType)  => Favorite.unapply(v).map { t => (
